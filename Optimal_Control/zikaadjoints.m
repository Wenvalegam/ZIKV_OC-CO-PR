function [ dlambda ] = zikaadjoints(t,lambda,tvec,x,u1,u2,phi,pars,fixpar,weights,IC)
...
%% Adjoint ODE system
dlambda=zeros(7,1);
    dlambda(1) = (lambda(1)-lambda(3))*(b1*x(7)*(1-u2)/Nv + a1*x(4)*(1-u1)/N) +...
    ...
    dlambda(4) = a1*x(1)*(1-u1)*(lambda(1)-lambda(3))/N + lambda(4)*(a2+mu) +...
        (lambda(5)-lambda(6))*b2*x(5)/N - A1;
    dlambda(5) = c1*lambda(5) + ((lambda(5)-lambda(6))*b2*x(4))/N;
    dlambda(6) = c2*(lambda(6)-lambda(7)) + c1*lambda(6);
    dlambda(7) = (lambda(1)-lambda(3))*b1*x(1)*(1-u2)/Nv + c1*lambda(7);
end
